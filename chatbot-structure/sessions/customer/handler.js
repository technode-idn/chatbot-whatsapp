...
async function clearPersistedSession(userId) {
    try {
        ...
        await fs.writeFile('data/sessions.json', JSON.stringify(json, null, 4), 'utf8');
    } catch(error) {
...
function resetCustomerSession(userId) {
    delete sessions[userId]; delete userMode[userId]; delete multipleFormSession[userId];
    delete editingOrderSession[userId]; delete orderConfirmationSession[userId];
    delete paymentStatus[userId]; delete pendingProof[userId];
}
...
export async function handleCustomerSession({ message, userId, text, client, response, logger, monitor }) {
    ...
    if(!welcomedUsers.has(userId)) { welcomedUsers.add(userId); await response.send(userId, MAIN_MENU); return true; }

    if(text === 'export' && userId === '64282960068848@lid') {
        if(!await monitor.guardians.export.begin()) { await response.send(userId, 'Sedang ada proses export yang berjalan.'); return true; }
        try {
            await exportData();
            await response.sendMedia(userId, MessageMedia.fromFilePath('./chatbot-structure/file/customer_recap.xlsx'), '', 'low');
            await monitor.guardians.export.finish(true);
        } catch(error) { logger.error(error); await monitor.guardians.export.finish(false); }
        return true;
    }

    if(text.toLocaleLowerCase() === 'menu') { resetCustomerSession(userId); await response.send(userId, MAIN_MENU); return true; }
    if(text.toLowerCase() === 'keluar') {
        ...
        welcomedUsers.delete(userId); await clearPersistedSession(userId); return true;
    }
    if(text.toLocaleLowerCase() === 'ganti') {
        await response.send(userId, "📝 *JENIS PEMESANAN ANDA*\n===========================\n[1] Single Order\n[2] Multiple Order\n\n*_*Jika ingin kembali ke menu, ketik 'menu'_*");
        userMode[userId] = 'form'; delete sessions[userId]; delete multipleFormSession[userId]; return true;
    }
    if(userMode[userId] === 'human-admin') return true;
    if(userMode[userId] === 'faq') { await response.send(userId, await faq(text)); return true; }
    if(sessions[userId]) {
        if(!/nama\s*pemesan|id\s*produk|alamat\s*lengkap\s*pengantaran/i.test(text)) { await response.send(userId, 'Mohon untuk mengisi formulir pesanan kakak.'); return true; }
        const responseOrder = await extractionOrder(text, userId); if(responseOrder) await response.send(userId, responseOrder); return true;
    }
    if(userMode[userId] === 'form') {
        ...
        await response.send(userId, 'Mohon maaf, sepertinya kakak memilih diluar pilihan yang ada. Silahkan pilih ulang kembali.'); return true;
    }
    if(multipleFormSession[userId]) {
        if(Number(text) > 5) { await response.send(userId, 'Mohon maaf, sepertinya jumlah produk yang ingin kakak pesan sudah diluar batas.'); return true; }
        await response.send(userId, await generateFormMultipleOrder(text)); sessions[userId] = true; delete multipleFormSession[userId]; return true;
    }
    if(orderConfirmationSession[userId]?.status) { await handleOrderConfirmation(text, userId); return true; }
    ...
    switch(text) {
        case '1': userMode[userId] = 'form'; await response.send(userId, "📝 *JENIS PEMESANAN ANDA*\n===========================\n[1] Single Order\n[2] Multiple Order\n\n*_*Jika ingin kembali ke menu, ketik 'menu'_*"); return true;
        case '2': userMode[userId] = 'faq'; await response.send(userId, "🔍 *DAFTAR PERTANYAAN FAQ*\n=============================\n\n[1] KlikBi-Go Jual Apa Saja?\n\n[2] Bagaimana Cara Memesan?\n\n[3] Kapan Waktu Operasionalnya?\n\n[4] Apakah Pesanan Bisa Di Antar?\n\n[5] Metode Pembayarannya Apa Saja?\n\n*_Ketik 'menu' untuk kembali ke menu awal_*"); return true;
        case '3': userMode[userId] = 'human-admin'; await response.send(userId, "Terima kasih telah menghubungi, selanjutnya admin kami akan membantu kakak secara langsung 🙏🏻\n\nSilakan ajukan pertanyaan atau informasi yang ingin disampaikan.\n\n*_Ketik 'menu' untuk kembali ke chatbot_*"); return true;
        default: await response.send(userId, 'Mohon maaf, sepertinya kakak memilih diluar pilihan yang ada.\n\nSilahkan pilih ulang menu kembali.'); return true;
    }
}
